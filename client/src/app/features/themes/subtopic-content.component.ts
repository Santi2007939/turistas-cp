                ...
                <div *ngFor="let problem of subtopic.linkedProblems; let k = index" 
                  ...
                  <div *ngIf="editingProblemIndex === k" class="space-y-3">
                    ...
                    <div class="flex gap-2 justify-end pt-1">
                      ...
                      <button
                        (click)="saveEditedProblem(problem)"
                        [disabled]="!editedProblem.title || savingProblem"
...
export class SubtopicContentComponent implements OnInit {
  ...
  } = { title: '', description: '', link: '', difficulty: 'easy' };
  savingProblem = false;
  ...
  saveEditedProblem(problem: SubtopicContent['linkedProblems'][0]): void {
    ...
    const transformedProblems = this.subtopic.linkedProblems?.map(p => ({
      ...p,
      problemId: p.problemId && p.problemId !== '' ? p.problemId : undefined
    }));
    this.themesService.updateSubtopicSharedContent(this.themeId, this.subtopicName, {
      ...
      error: (err) => {
        this.savingProblem = false;
        this.error = 'Failed to save problem. Please try again.';
